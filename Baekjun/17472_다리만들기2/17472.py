def check_island(x, y, n): #2번부터 섬 네이밍
    if not field[x][y]:
        DFS_island(x, y, n)
        return n + 1
    return n

def DFS_island(x, y, n): #섬을 다니며 좌표를 수집해 lands에 섬별 좌표를 저장
    s = []
    v = []
    s.append((x, y))
    while s:
...
def check_bridge(n1, n2): #다리후보만들기
    ret = []
    for x, y in lands[n1]:
        for k in range(4): #4방향서치
            temp = []
            ...
            while 0 <= x+dx[k]*i <= N-1 and 0 <= y+dy[k]*i <= M-1: #경계끝까지 
                if (field[x+dx[k]*i][y+dy[k]*i] >= 2 and field[x+dx[k]*i][y+dy[k]*i] != n2): #내 땅을 또 만나면 다음 방향 서치
                    break
...
for tc in range(1, T+1):
    ...
    field = [list(False if x == '1' else True for x in input().split()) for _ in range (N)]
    lands = [[], []]
    bridges = []
    n = 2
    for x in range(N):
