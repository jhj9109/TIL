...
def go(x, y):#반복버전
    ...
    while s:
        ...
        for k in range(4): #4방향 서치
            if 0<=x+dx[k]<=99 and 0<=y+dy[k]<=99 and field[x+dx[k]][y+dy[k]] != 1 and visited[x+dx[k]][y+dy[k]] == False: #방문했던곳이 아니면
                s.append(x+dx[k]) #방문할 칸 목록에 push, 갈림길 1개 -> 그 길로, 갈림길 2길 -> 저장.
                s.append(y+dy[k]) 
